...
//        assert(mConv9_2_mbox_loc_perm_layer.get() == nullptr);
    ...
    else if (!strcmp(layerName, "detection_out"))
    {
		...
		// {true,false,0,21,400,200,0.1,4.5,CodeTypeSSD::CENTER_SIZE} = params
        mDetection_out = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>
                (nvinfer1::plugin::createSSDDetectionOutputPlugin(params), nvPluginDeleter);
        std::cout<<" ---convert detection_out pluginlayer--- " <<std::endl;
